...
\cs_new_eq:NN \clist_gset:Nn \tl_gset:Nn
\cs_new_protected:Nn \clist_const:Nn {
  \clist_new:N #1
  \clist_gset:Nn #1 { #2 }
}
...
\luatex_if_engine:TF {
  \cs_new_protected_nopar:Nn \load_opentype_font:nn {
    \font_set:Nnn \l_params_font { { file: #1 (#2) } } { 10pt }
  }
} {
  \xetex_if_engine:T {
    \cs_new_protected_nopar:Nn \load_opentype_font:nn {
      \font_set:Nnn \l_params_font { "[ #1 ]" } { 10pt }
    }
...
\cs_new_protected_nopar:Nn \write_param:nN {
  ...
  \cs_if_exist:cT { fontparams_style_get_ #1 :N } {
